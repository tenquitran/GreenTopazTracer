...
std::vector< std::pair<VComponent, VComponent> > Sampler::getSamples_Jittered(VComponent pixelX, VComponent pixelY)
{
	...
	for (int i = 0; i < SampleCount; ++i)
	{
		...
		samples.push_back(std::pair<VComponent, VComponent>(xDist(m_randomEngine), yDist(m_randomEngine)));
	}

	return samples;
}

std::vector< std::pair<VComponent, VComponent> > Sampler::getSamples_Jittered1()
{
	std::vector< std::pair<VComponent, VComponent> > samples;
	samples.reserve(SampleCount);
